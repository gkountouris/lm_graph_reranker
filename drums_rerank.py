...
from modeling import modeling_drums_rerank
from utils import data_utils_reranker
...
def load_data(args, devices, kg):
    ...
    if args.local_rank != -1 and one_process_at_a_time:
        for p_rank in range(args.world_size):
            ...
            dataset = data_utils_reranker.DRUMS_DataLoader(args, args.train_statements, args.train_adj, args.train_tensor,
...
def construct_model(args, kg, dataset):
    ...
    model = modeling_drums_rerank.DRUMS(args, encoder_load_path, k=args.k, n_ntype=n_ntype, n_etype=n_etype, n_concept=concept_num,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_tensor', default=f'{args.data_dir}/{args.dataset}/tensors/train_midle.saved_tensors.pt', help="The path to the tf-idf tesnsors of the training set.")
    parser.add_argument('--dev_tensor', default=f'{args.data_dir}/{args.dataset}/tensors/dev_midle.saved_tensors.pt', help="The path to the tf-idf tesnsors of the dev set.")
    parser.add_argument('--test_tensor', default=f'{args.data_dir}/{args.dataset}/tensors/test_midle.saved_tensors.pt', help="The path to the tf-idf tesnsors of the test set.")

    parser.add_argument('--train_adj', default=f'{args.data_dir}/{args.dataset}/graph/train_midle.graph.adj.pk', help="The path to the retrieved KG subgraphs of the training set.")
    parser.add_argument('--dev_adj', default=f'{args.data_dir}/{args.dataset}/graph/dev_midle.graph.adj.pk', help="The path to the retrieved KG subgraphs of the dev set.")
    parser.add_argument('--test_adj', default=f'{args.data_dir}/{args.dataset}/graph/test_midle.graph.adj.pk', help="The path to the retrieved KG subgraphs of the test set.")
