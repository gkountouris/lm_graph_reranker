...
from modeling import modeling_drums_retriever
...
def construct_model(args, kg, dataset):
    ...
    model = modeling_drums_retriever.DRUMS(args, encoder_load_path, k=args.k, n_ntype=n_ntype, n_etype=n_etype, n_concept=concept_num,
...
def calc_retrieval_loss_and_acc(queries_retrieved_docs, queries_relevant_docs):
    loss = 0.
    n_corrects = 0
    mrr_metric_5 = metric_utils.mean_reciprocal_rank_at_k_tensor(queries_retrieved_docs, queries_relevant_docs, 5)
    mrr_metric_10 = metric_utils.mean_reciprocal_rank_at_k_tensor(queries_retrieved_docs, queries_relevant_docs, 10)
    map_10 = metric_utils.calculate_map(queries_retrieved_docs, queries_relevant_docs, 10)
    return loss, n_corrects, mrr_metric_5, mrr_metric_10, map_10


def calc_eval_accuracy(args, eval_set, model, loss_type, loss_func, debug, save_test_preds, preds_path, devices):
    ...
    mrr_metric_5_acm = mrr_metric_10_acm = map_10 = 0.0
        ...
        for qids, labels, *input_data in tqdm(eval_set, desc="Dev/Test batch"):
            ...
            if args.fp16:
                with torch.cuda.amp.autocast():
                    ...
                    end_loss, n_corrects, mrr_metric_5, mrr_metric_10, map_10 = calc_retrieval_loss_and_acc(sorted_doc_labels_tensor.to(devices[0]), labels)
            else:
                logits, retrieval_loss, num_retrieved, sorted_doc_labels_tensor, mlm_loss, link_losses = model(*input_data, labels=labels)
                end_loss, n_corrects, mrr_metric_5, mrr_metric_10, map_10 = calc_retrieval_loss_and_acc(sorted_doc_labels_tensor.to(devices[0]), labels)
    ...
    total_loss_avg, end_loss_avg, retrieval_loss_avg, mlm_loss_avg, link_loss_avg, pos_link_loss_avg, neg_link_loss_avg, n_corrects_avg, n_correct_docs_avg, mrr_metric_5_avg, mrr_metric_10_avg, map_10_avg = \
        [item / n_samples_acm for item in (total_loss_acm, end_loss_acm, retrieval_loss_acm, mlm_loss_acm, link_loss_acm, pos_link_loss_acm, neg_link_loss_acm, n_corrects_acm, n_corrects_docs_acm, mrr_metric_5_acm, mrr_metric_10_acm, map_10_acm)]
    return total_loss_avg, end_loss_avg, retrieval_loss_avg, mlm_loss_avg, link_loss_avg, pos_link_loss_avg, neg_link_loss_avg, n_corrects_avg, n_correct_docs_avg, mrr_metric_5_avg, mrr_metric_10_avg, map_10_avg
...
def evaluate(args, has_test_split, devices, kg):
    ...
    model.lmgnn.cls_projection.to(devices[1])
    model.lmgnn.gnn_projection.to(devices[1])
    ...
    dev_total_loss, dev_end_loss, dev_retrieval_loss, dev_mlm_loss, dev_link_loss, dev_pos_link_loss, dev_neg_link_loss, dev_acc, dev_docs_acc, dev_mrr_metric_5, dev_mrr_metric_10, dev_map_10  = calc_eval_accuracy(args, dev_dataloader, model, args.loss, loss_func, debug, not debug, preds_path, devices)

    if has_test_split:
        # Evaluation on the test set
        preds_path = os.path.join(args.save_dir, 'test_e{}_preds.pt'.format(epoch_id))
        test_total_loss, test_end_loss, test_retrieval_loss, test_mlm_loss, test_link_loss, test_pos_link_loss, test_neg_link_loss, test_acc, test_docs_acc, test_mrr_metric_5, test_mrr_metric_10, test_map_10 = calc_eval_accuracy(args, test_dataloader, model, args.loss, loss_func, debug, not debug, preds_path, devices)
    else:
        test_acc = 0

    print('-' * 71)
    print('dev_acc {:7.4f} | dev_mrr_5 {:7.4f} | dev_mrr_10 {:7.4f} | dev_map_10 {:7.4f}, test_acc {:7.4f} | test_mrr_5 {:7.4f} | test_mrr_10 {:7.4f}| test_map_10 {:7.4f}'.format(dev_docs_acc, dev_mrr_metric_5, dev_mrr_metric_10, dev_map_10, test_docs_acc, test_mrr_metric_5, test_mrr_metric_10, test_map_10))
    print('-' * 71)
    ...
    if not resume:
        with open(log_path, 'w') as fout:
            fout.write('epoch_id, global_step, dev_docs_acc, test_docs_acc, best_dev_acc, final_test_acc, dev_mrr_metric_5, test_mrr_metric_5, dev_mrr_metric_10, test_mrr_metric_10, dev_map_10, test_map_10, best_dev_epoch\n')

    with open(log_path, 'a') as fout:
        fout.write('{:3},{:5},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:7.4f},{:3}\n'.format(epoch_id, global_step, dev_docs_acc, test_docs_acc, best_dev_acc, final_test_acc, dev_mrr_metric_5, test_mrr_metric_5, dev_mrr_metric_10, test_mrr_metric_10, dev_map_10, test_map_10, best_dev_epoch))
...
def train(args, resume, has_test_split, devices, kg):
    ...
    elif args.lr_schedule == 'warmup_linear' or resume:
    ...
    for epoch_id in trange(0, args.n_epochs, desc="Epoch", disable=args.local_rank not in [-1, 0]): #trange(last_epoch + 1, args.n_epochs, desc="Epoch"):
        ...
        for qids, labels, *input_data in tqdm(dataset.train(steps=args.redef_epoch_steps, local_rank=args.local_rank), desc="Batch", disable=args.local_rank not in [-1, 0]): #train_dataloader
            ...
            for _idx_, a in enumerate(a_list):
                ...
                if args.fp16:
                    with torch.cuda.amp.autocast():
                        ...
                        # end_loss, n_corrects, mrr_metric_5, mrr_metric_10 = calc_retrieval_loss_and_acc(sorted_doc_labels_tensor.to(devices[0]), labels[a:b])
                else:
                    logits, retrieval_loss, num_retrieved, sorted_doc_labels_tensor, mlm_loss, link_losses = model(*[x[a:b] for x in input_data], labels=labels[a:b]) # logits: [bs, nc] and tf_idf_matrix
                    # end_loss, n_corrects, mrr_metric_5, mrr_metric_10 = calc_retrieval_loss_and_acc(sorted_doc_labels_tensor.to(devices[0]), labels[a:b])
                link_loss, pos_link_loss, neg_link_loss = link_losses
            ...
            if (global_step + 1) % args.log_interval == 0:
                ...
                if args.local_rank in [-1, 0]:
                    # print('| step {:5} |  lr: {:9.7f} | total loss {:7.4f} | mrr_metric_5 {:7.4f} | mrr_metric_10 {:7.4f} | ms/batch {:7.2f} |'.format(global_step, scheduler.get_lr()[0], total_loss_acm / n_samples_acm, mrr_metric_5_acm / n_samples_acm, mrr_metric_10_acm / n_samples_acm, ms_per_batch))
        ...
        if args.local_rank in [-1, 0]:
            model.eval()
            preds_path = os.path.join(args.save_dir, 'dev_e{}_preds.pt'.format(epoch_id))
            dev_total_loss, dev_end_loss, dev_retrieval_loss, dev_mlm_loss, dev_link_loss, dev_pos_link_loss, dev_neg_link_loss, dev_acc, dev_docs_acc, dev_mrr_metric_5, dev_mrr_metric_10, dev_map_10 = calc_eval_accuracy(args, dev_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path, devices)
            print ('dev_docs_acc', dev_docs_acc, file=sys.stderr)

            if has_test_split:
                preds_path = os.path.join(args.save_dir, 'test_e{}_preds.pt'.format(epoch_id))
                test_total_loss, test_end_loss, test_retrieval_loss, test_mlm_loss, test_link_loss, test_pos_link_loss, test_neg_link_loss, test_acc, test_docs_acc, test_mrr_metric_5, test_mrr_metric_10, test_map_10 = calc_eval_accuracy(args, test_dataloader, model, args.loss, loss_func, args.debug, not args.debug, preds_path, devices)
                print ('test_docs_acc', test_docs_acc, file=sys.stderr)
            else:
                test_docs_acc = 0
                test_retrieval_loss = 0

            print('-' * 71)
            print('| epoch {:3} | step {:5} | dev_docs_acc {:7.4f} | test_docs_acc {:7.4f} | dev_retrieval_loss {:7.4f} | test_retrieval_loss {:7.4f} | dev_mrr_metric_5 {:7.4f} | test_mrr_metric_5 {:7.4f} | dev_mrr_metric_10 {:7.4f} | dev_map_10 {:7.4f} | test_mrr_metric_10 {:7.4f} | test_map_10 {:7.4f}'.format(epoch_id, global_step, dev_docs_acc, test_docs_acc, dev_retrieval_loss, test_retrieval_loss, dev_mrr_metric_5, test_mrr_metric_5, dev_mrr_metric_10, dev_map_10, test_mrr_metric_10, test_map_10))
            print('-' * 71)

            if dev_mrr_metric_10 >= best_dev_mrr:
                best_dev_mrr = dev_mrr_metric_10
            ...
            if ((args.save_model==1) and (best_dev_epoch==epoch_id)) or epoch_id==0:
                if args.local_rank != -1:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_model', default=1, type=float, help="0: do not save model checkpoints. 1: save if best dev.")
    ...
    parser.add_argument('--train_tensor', default=f'{args.data_dir}/{args.dataset}/tensors/training11b.saved_tensors.pt', help="The path to the tf-idf tesnsors of the training set.")
    parser.add_argument('--dev_tensor', default=f'{args.data_dir}/{args.dataset}/tensors/11B1_golden.saved_tensors.pt', help="The path to the tf-idf tesnsors of the dev set.")
    parser.add_argument('--test_tensor', default=f'{args.data_dir}/{args.dataset}/tensors/11B3_golden.saved_tensors.pt', help="The path to the tf-idf tesnsors of the test set.")

    parser.add_argument('--train_adj', default=f'{args.data_dir}/{args.dataset}/graph/training11b.graph.adj.pk', help="The path to the retrieved KG subgraphs of the training set.")
    parser.add_argument('--dev_adj', default=f'{args.data_dir}/{args.dataset}/graph/11B1_golden.graph.adj.pk', help="The path to the retrieved KG subgraphs of the dev set.")
    parser.add_argument('--test_adj', default=f'{args.data_dir}/{args.dataset}/graph/11B3_golden.graph.adj.pk', help="The path to the retrieved KG subgraphs of the test set.")
