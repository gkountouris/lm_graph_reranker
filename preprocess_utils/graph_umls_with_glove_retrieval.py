...
def concepts_to_adj_matrices_2hop_all_pair__use_glove__Part1(data):
    ...
    results = elastic_search_query.elastic_search_text(question, 5)
    bm_ids = set()
    pmid = []
    for res in results['hits']['hits']:
        pmid.append(res['_source']['PMID'])
    ...
    extra_nodes = extra_nodes - qc_ids
    extra_nodes = set(extra_nodes) | set(bm_ids)
    # print(question, len(extra_nodes), pmid)
...
#         if qid in cpnet_simple.nodes:
#             extra_nodes |= set(cpnet_simple[qid])
#     # qa_nodes = extra_nodes
#     # for qid in extra_nodes:
#     #     for aid in qa_nodes:
#     #         if qid != aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
#     #             extra_nodes |= set(cpnet_simple[qid]) & set(cpnet_simple[aid])
#     extra_nodes = extra_nodes - qc_ids
...
def concepts_to_adj_matrices_2hop_all_pair__use_glove__Part2(data):
    ...
    cid2score = get_glove_score(qc_ids+extra_nodes, question)
...
def concepts_to_adj_matrices_2hop_all_pair__use_glove__Part3(data):
    ...
    schema_graph = qc_ids + sorted(extra_nodes, key=lambda x: -cid2score[x])[:200] #score: from high to low
...
def generate_adj_data_from_grounded_concepts_umls_retrieval__use_glove(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    """
