...
class DRUMS(nn.Module):
    ...
    def forward(self, *inputs, labels=None, k=0.5, l=0.5, cache_output=False, detail=False):
        ...
        assert len(inputs) == 6 + 5 + 2 + 2 + 1 + 1 + 1 + 1 + 1 # 18 lm_data, 5 gnn_data, (edge_index, edge_type), (pos_triples, neg_nodes), doc_dense_tensor, doc_labels_tensor, embeddings, bm25_score_tensor, emb_reduction(V)
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type, pos_triples, neg_nodes = _inputs
        ...
        logits, retrieval_loss, sorted_doc_labels_tensor1, sorted_doc_labels_tensor2, sorted_doc_labels_tensor3, sorted_node_labels_tensor, lm_loss, link_losses = self.lmgnn(lm_inputs, concept_ids,
                                    node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, lp_data, labels, doc_dense_tensor, doc_labels_tensor, embeddings, bm25_score_tensor,
...
class LMGNN(PreTrainedModelClass):
    ...
    def forward(self, inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, lp_data, labels, doc_dense_tensor, doc_labels_tensor, doc_embeddings_tensor, bm25_score_tensor, k_param, l_param, V, emb_data=None, cache_output=False):
        """
        ...
        node_mask_retrieval = node_mask | (node_type_ids == 2)
        ...
        if self.args.retrieval_task:
            ...
            # doc_dense_tensor = doc_dense_tensor != 0  # This will create a boolean tensor where True represents non-zero values.
            # doc_dense_tensor = doc_dense_tensor.float()  # Convert boolean tensor back to float, turning True to 1.0 and False to 0.0.
            ...
            # cls_doc_embeddings = self.embeddings_projection(doc_embeddings_tensor)

            # combined_embeddings = torch.cat([cls_doc_embeddings, gnn_doc_embeddings], dim=-1)

            # final_embeddings = self.combined_projection(combined_embeddings)
            final_embeddings = gnn_doc_embeddings

            # Compute cosine node similarity
            node_similarities = F.cosine_similarity(cls_token_nodes_expanded, gnn_output, dim=2)
            node_similarities = node_similarities.to(bm25_score_tensor.device)

            # Compute cosine similarity
            similarities = F.cosine_similarity(cls_token_expanded, final_embeddings, dim=2)
            similarities = similarities.to(bm25_score_tensor.device)
            ...
            k_param = 0.0
            l_param = 1.0
            final_scores1 = k_param*similarities + l_param*bm25_score_tensor

            k_param = 0.6
            l_param = 1.0
            final_scores2 = k_param*similarities + l_param*bm25_score_tensor
            
            k_param = 1.0
            l_param = 0.0
            final_scores3 = k_param*similarities + l_param*bm25_score_tensor

            # print("l_param*bm25_score_tensor", l_param*bm25_score_tensor, file=sys.stderr)

            # similarities = similarities.transpose(0, 1).unsqueeze(-1)
            # bm25_scores = bm25_score_tensor.transpose(0, 1).unsqueeze(-1)

            # final_scores, attn_output_weights = self.bm25_attention(query=similarities, key=bm25_scores, value=bm25_scores)
            # final_scores = final_scores.squeeze(-1).transpose(0, 1)
            
            # final_scores = self.min_max_score_normalize_tensor(final_scores)

            # combined_input_scores = torch.stack((final_scores, bm25_score_tensor), dim=-1)

            # final_scores = self.score_combiner(combined_input_scores)
            ...
            binary_mask = self.create_match_mask(doc_labels_tensor, stacked_labels)

            # print("gnn_doc_embeddings", gnn_doc_embeddings[binary_mask], file=sys.stderr)
            
            retrieval_loss = self.combined_loss(similarities, binary_mask)
    ...
    def combined_loss(self, final_scores, binary_mask, margin_contrastive=0.65, margin_ranking=0.2, alpha=0.0):
        """
        ...
        retrieval_loss = self.retrieval_loss_fnc(final_scores, binary_mask)
        ranking_loss = self.ranking_similarity_loss(final_scores, binary_mask, margin=margin_ranking)

        retrieval_loss = torch.where(torch.isnan(retrieval_loss), torch.zeros_like(retrieval_loss), retrieval_loss)
        ranking_loss = torch.where(torch.isnan(ranking_loss), torch.zeros_like(ranking_loss), ranking_loss)

        combined_loss = alpha * retrieval_loss + (1 - alpha) * ranking_loss
    ...
    def ranking_similarity_loss(self, similarities, binary_mask, margin=0.1):
        ...
        true_counts = torch.where(true_counts == 0, torch.tensor(1.0), true_counts)
