...
class DRUMS(nn.Module):

    def __init__(self, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
                 n_concept=799273, concept_dim=200, concept_in_dim=1024,
                 n_attention_head=2, fc_dim=200, n_fc_layer=0, p_emb=0.2, p_gnn=0.2, p_fc=0.2,
    ...
    def forward(self, *inputs, labels=None, k=0.5, l=0.5, cache_output=False, detail=False):
        """
        ...
        assert len(inputs) == 6 + 5 + 2 + 2 + 1 + 1 + 1 + 1 + 1 + 1 # 18 lm_data, 5 gnn_data, (edge_index, edge_type), (pos_triples, neg_nodes), doc_dense_tensor, doc_labels_tensor, embeddings, bm25_score_tensor, emb_reduction(V)
        edge_index_orig, edge_type_orig = inputs[13:15]

        doc_dense_tensor = inputs[15]
        doc_labels_tensor = inputs[16]
        embeddings = inputs[17]
        bm25_score_tensor = inputs[18]
        V = inputs[19]
        relevant_ent = inputs[20]
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, edge_index, edge_type, pos_triples, neg_nodes = _inputs
        ...
        logits, retrieval_loss, sorted_doc_labels_tensor1, sorted_doc_labels_tensor2, sorted_doc_labels_tensor3, sorted_node_labels_tensor, lm_loss, link_losses = self.lmgnn(lm_inputs, concept_ids,
                                    node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, lp_data, labels, doc_dense_tensor, doc_labels_tensor, embeddings, bm25_score_tensor, relevant_ent,
                                    k, l, V, emb_data=None, cache_output=cache_output)
        ...
        if not detail:
            return logits, retrieval_loss, sorted_doc_labels_tensor1, sorted_doc_labels_tensor2, sorted_doc_labels_tensor3, sorted_node_labels_tensor, lm_loss, link_losses
        else:
            return logits, retrieval_loss, sorted_doc_labels_tensor1, sorted_doc_labels_tensor2, sorted_doc_labels_tensor3, sorted_node_labels_tensor, lm_loss, link_losses, concept_ids.view(bs, nc, -1), node_type_ids.view(bs, nc, -1), edge_index_orig, edge_type_orig
...
def test_DRUMS(device):
    cp_emb = torch.load("models/biomed_model.pt")
    print("testing the Drums....")
    model = DRUMS(pretrained_concept_emb=cp_emb).to(device)
...
class LMGNN(PreTrainedModelClass):
    ...
    def __init__(self, config, args={}, model_name="roberta-large", k=5, n_ntype=4, n_etype=38,
        ...
        self.pooler_retrieve = layers.MultiheadAttPoolLayer(n_attention_head, config.hidden_size, concept_dim)
        self.fc_retrieve = layers.MLP(concat_vec_dim, fc_dim, 1, n_fc_layer, p_fc, layer_norm=True)
        ...
        if args.retrieval_task:
            self.cls_projection = layers.MLP(concept_in_dim, concept_in_dim, concept_dim, 1, 0.3, layer_norm=True)
            self.gnn_projection = layers.MLP(concept_dim, concept_dim, concept_dim, 1, 0.3, layer_norm=True)
            # self.tokens_attention = layers.CrossAttention() get_last_lr()
            # self.documents_projection = layers.MLP(concept_dim, concept_dim, concept_dim, 1, 0., layer_norm=False)
            # self.embeddings_projection = layers.MLP(concept_in_dim, concept_in_dim, concept_dim, 3, 0., layer_norm=False)
            # self.combined_projection = layers.MLP(2*concept_dim, concept_dim, concept_dim, 1, 0., layer_norm=False)
            # self.combine_scores = layers.MLP(2, 64, 1, 1, 0.3, layer_norm=True)
            # self.score_combiner = layers.ScoreCombinerNN(hidden_size1=64, hidden_size2=32, dropout_p= 0.3)
            # self.bm25_attention = torch.nn.MultiheadAttention(embed_dim=1, num_heads=1)
    ...
    def forward(self, inputs, concept_ids, node_type_ids, node_scores, adj_lengths, special_nodes_mask, adj, lp_data, labels, doc_dense_tensor, doc_labels_tensor, doc_embeddings_tensor, bm25_score_tensor, relevant_binary, k_param, l_param, V, emb_data=None, cache_output=False):
        """
        concept_ids: (batch_size, n_node)
        adj_lengths: (batch_size,)
        node_type_ids: (batch_size, n_node)
            0 == question entity; 1 == document entity; 2 == other node; 3 == context node
        ...
        if self.args.retrieval_task:
            ...
            num_rerank_labels = doc_dense_tensor.shape[1]
            max_num_nodes = 5000

            sums = doc_dense_tensor.sum(dim=-1, keepdim=True)
            # Avoid division by zero by ensuring sums are not zero
            sums = sums.where(sums != 0, torch.ones_like(sums))
            doc_dense_tensor = doc_dense_tensor / sums

            # print("V", V.shape, file=sys.stderr)

            # V = V.to(doc_dense_tensor.device)

            cls_embs = self.cls_projection(sent_vecs.to(doc_dense_tensor.device))
            # k = 200  # number of components to keep
            # cls_embs = torch.matmul(sent_vecs.to(doc_dense_tensor.device), V[:k, :])

            # Expand the CLS token to match the embeddings' shape
            cls_token_expanded = cls_embs.unsqueeze(1).expand(-1, num_rerank_labels, -1)
            cls_token_nodes_expanded = cls_embs.unsqueeze(1).expand(-1, max_num_nodes, -1)

            # Exclude also the context nodes or node
            gnn_output = gnn_output * (~node_mask_retrieve).float().unsqueeze(2)

            # print("sent_vecs", sent_vecs, file=sys.stderr)
            # print("sent_vecs", sent_vecs.shape, file=sys.stderr)
            # print("gnn_output", gnn_output, file=sys.stderr)
            # print("gnn_output", gnn_output.shape, file=sys.stderr)
            # print("node_mask_retrieve", node_mask_retrieve, file=sys.stderr)
            # print("node_mask_retrieve", node_mask_retrieve.shape, file=sys.stderr)
            # print("node_mask", node_mask, file=sys.stderr)
            # print("node_mask", node_mask.shape, file=sys.stderr)

            # if self.k >= 0:
            #     graph_vecs, pool_attn = self.pooler_retrieve(sent_vecs, gnn_output, node_mask) #graph_vecs: [bs, node_dim]
            #     concat_pool = torch.cat((graph_vecs, sent_vecs, Z_vecs), 1)
            #     print("graph_vecs", graph_vecs, file=sys.stderr)
            #     print("Size of graph_vecs", graph_vecs.size(), file=sys.stderr)
            #     print("pool_attn", pool_attn, file=sys.stderr)
            #     print("pool_attn", pool_attn.shape, file=sys.stderr)
            #     print("concat_pool", concat_pool, file=sys.stderr)
            #     print("concat_pool", concat_pool.shape, file=sys.stderr)
            # else:
            #     concat_pool = sent_vecs
            # logits = self.fc_retrieve(self.dropout_fc(concat_pool)) #[bs, 1]

            # print("logits", logits, file=sys.stderr)
            # print("logits", logits.shape, file=sys.stderr)

            gnn_output = gnn_output.to(doc_dense_tensor.device)

            # # Process GNN output
            # gnn_output = self.process_gnn_output(gnn_output, doc_dense_tensor.device)

            # Matrix multiplication, produce doc embeddings
            # gnn_doc_embeddings = torch.bmm(doc_dense_tensor, gnn_output)
            # gnn_doc_embeddings = self.documents_projection(gnn_doc_embeddings)

            # Compute cosine node similarity
            node_similarities = F.cosine_similarity(cls_token_nodes_expanded, gnn_output.to(doc_dense_tensor.device), dim=2)
            node_similarities = node_similarities.unsqueeze(-1)

            # # Compute cosine similarity
            # gnn_doc_similarities = F.cosine_similarity(cls_token_expanded, gnn_doc_embeddings, dim=2)
            # gnn_doc_similarities = gnn_doc_similarities.to(bm25_score_tensor.device)

            gnn_doc_similarities = torch.bmm(doc_dense_tensor, node_similarities)
            gnn_doc_similarities = gnn_doc_similarities.to(bm25_score_tensor.device)
            gnn_doc_similarities = gnn_doc_similarities.squeeze()
            node_similarities = node_similarities.squeeze().to(bm25_score_tensor.device)

            # print("bm25_score_tensor", bm25_score_tensor, file=sys.stderr)
            # print("bm25_score_tensor", bm25_score_tensor.shape, file=sys.stderr)

            # print("gnn_doc_similarities", gnn_doc_similarities, file=sys.stderr)
            # print("gnn_doc_similarities", gnn_doc_similarities.shape, file=sys.stderr)

            # similarities = self.min_max_score_normalize_tensor(similarities)                      
            bm25_score_tensor = self.min_max_score_normalize_tensor(bm25_score_tensor)

            k_param = 0.0
            l_param = 1.0
            final_scores1 = k_param*gnn_doc_similarities + l_param*bm25_score_tensor

            k_param = 1.0
            l_param = 1.0
            final_scores2 = k_param*gnn_doc_similarities + l_param*bm25_score_tensor
            
            k_param = 1.0
            l_param = 0.0
            final_scores3 = k_param*gnn_doc_similarities + l_param*bm25_score_tensor

            max_size = max(tensor.size(0) for tensor in labels)
            padded_labels = [torch.nn.functional.pad(tensor, (0, max_size - tensor.size(0))) for tensor in labels]
            stacked_labels = torch.stack(padded_labels)

            binary_mask_doc = self.create_match_mask(doc_labels_tensor, stacked_labels)
            # print("binary_mask_doc", binary_mask_doc, file=sys.stderr)
            
            retrieval_loss = self.combined_loss(node_similarities, relevant_binary.to(node_similarities.device), gnn_doc_similarities, binary_mask_doc)

            # print("doc_labels_tensor", doc_labels_tensor, file=sys.stderr)
            # print("doc_labels_tensor", doc_labels_tensor.shape, file=sys.stderr)

            # # Sort the similarities and get the indices
            sorted_final_scores, sorted_indices = torch.sort(final_scores1, descending=True)  # Assuming you want descending order
            # print("sorted_indices", sorted_indices, file=sys.stderr)
            # print("sorted_indices", sorted_indices.shape, file=sys.stderr)
            # Use the sorted indices to reorder doc_labels_tensor
            sorted_doc_labels_tensor1 = doc_labels_tensor.gather(1, sorted_indices)

            # # Sort the similarities and get the indices
            sorted_final_scores, sorted_indices = torch.sort(final_scores2, descending=True)  # Assuming you want descending order
            # Use the sorted indices to reorder doc_labels_tensor
            sorted_doc_labels_tensor2 = doc_labels_tensor.gather(1, sorted_indices)

            # # Sort the similarities and get the indices
            sorted_final_scores, sorted_indices = torch.sort(final_scores3, descending=True)  # Assuming you want descending order
            # Use the sorted indices to reorder doc_labels_tensor
            sorted_doc_labels_tensor3 = doc_labels_tensor.gather(1, sorted_indices)

            # # Sort the similarities and get the indices
            sorted_final_scores, sorted_indices = torch.sort(node_similarities, descending=True)  # Assuming you want descending order
            # Use the sorted indices to reorder doc_labels_tensor

            # Example tensor initialization (simulating shapes)
            batch_size = concept_ids.shape[0]  # e.g., 1 or any dynamic size
            num_features = concept_ids.shape[1]  # e.g., 5000

            # Ensure sorted_indices is of the correct shape
            # Check if sorted_indices is 1D and concept_ids is 2D
            if sorted_indices.dim() == 1 and concept_ids.dim() == 2:
                # Add a dimension to sorted_indices to make it [1, num_features]
                sorted_indices = sorted_indices.unsqueeze(0)

                # If the batch size of concept_ids is more than 1, we need to expand sorted_indices
                if batch_size > 1:
                    # Expand sorted_indices to match the batch size of concept_ids
                    sorted_indices = sorted_indices.expand(batch_size, -1)  # -1 means keeping the original size in that dimension

            # Now you can perform the gather operation safely
            sorted_node_labels_tensor = concept_ids.gather(1, sorted_indices)
        ...
        return logits, retrieval_loss, sorted_doc_labels_tensor1, sorted_doc_labels_tensor2, sorted_doc_labels_tensor3, sorted_node_labels_tensor, lm_loss, (link_loss, pos_link_loss, neg_link_loss)

    def z_score_normalize_tensor(self, scores):
        mean_val = torch.mean(scores)
        std_val = torch.std(scores)
        normalized_scores = (scores - mean_val) / std_val
        return normalized_scores

    def min_max_score_normalize_tensor(self, scores, new_min=0, new_max=1):
        """
        Applies min-max normalization to the input tensor for each batch independently.

        Parameters:
        - scores: Input tensor of scores to be normalized. Assumes the first dimension is the batch dimension.
        - new_min: The lower bound of the scaled range. (default: 0)
        - new_max: The upper bound of the scaled range. (default: 1)

        Returns:
        - normalized_scores: Tensor of the same shape as input, with values scaled to [new_min, new_max] for each batch.
        """
        min_val = torch.min(scores, dim=1, keepdim=True)[0]
        max_val = torch.max(scores, dim=1, keepdim=True)[0]
        
        # Avoid division by zero in case all scores are the same for a batch
        denom = max_val - min_val
        denom[denom == 0] = 1  # Prevent division by zero by replacing 0s with 1s
        
        normalized_scores = (scores - min_val) / denom * (new_max - new_min) + new_min
        
        return normalized_scores

    def process_gnn_output(self, gnn_output, device):
        _bs, max_node_num, gnn_dim = gnn_output.size()
        gnn_output = gnn_output.view(-1, gnn_dim).to(device)
        gnn_output = self.gnn_projection(gnn_output)
        return gnn_output.view(_bs, max_node_num, gnn_dim)

    def create_match_mask(self, doc_labels_tensor, labels):
        # Ensure labels is a set for faster lookup
        labels_set = [set(labels[i].tolist()) for i in range(labels.size(0))]
        
        # Prepare a tensor for the result, initially filled with zeros
        mask = torch.zeros_like(doc_labels_tensor, dtype=torch.bool)
        
        # Iterate through each batch item
        for i in range(doc_labels_tensor.size(0)):
            # Check each doc_label against the labels set for this batch item
            for j, doc_label in enumerate(doc_labels_tensor[i]):
                if doc_label.item() in labels_set[i]:
                    mask[i, j] = 1
                    
        return mask


    def combined_loss(self, ent_sim, binary_mask_ent, doc_similarities, binary_mask_doc, margin_ranking=0.2, alpha = 0.3):
        """
        Combines contrastive loss with similarity scores and custom ranking loss.
        
        Parameters:
        - similarity_matrix: Tensor of shape [N, N], similarity scores between pairs.
        - final_scores: Tensor of shape [batch_size, num_documents], scores for document ranking.
        - labels: Tensor of shape [N], indicating positive (1) and negative (0) pairs.
        - binary_mask: Boolean tensor indicating relevancy of documents.
        - margin_contrastive: Margin for the contrastive loss.
        - margin_ranking: Margin for the ranking loss.
        - alpha: Weighting factor for balancing the two losses (0 <= alpha <= 1).
        
        Returns:
        - combined_loss: Scalar tensor representing the combined loss.
        """

        entities_loss = self.ranking_similarity_loss(ent_sim, binary_mask_ent, margin=margin_ranking)
        document_loss = self.ranking_similarity_loss(doc_similarities, binary_mask_doc, margin=margin_ranking)

        # Check for NaNs
        if torch.isnan(entities_loss).any() or torch.isnan(document_loss).any():
            print("NaNs detected in the loss calculation.", file=sys.stderr)

        entities_loss = torch.where(torch.isnan(entities_loss), torch.zeros_like(entities_loss), entities_loss)
        document_loss = torch.where(torch.isnan(document_loss), torch.zeros_like(document_loss), document_loss)

        combined_loss = alpha * entities_loss + (1 - alpha) * document_loss
        # combined_loss = entities_loss + document_loss

        return combined_loss.sum()

    def retrieval_loss_fnc(self, similarities, binary_mask):

        # Convert binary_mask to boolean for indexing
        positive_mask = binary_mask.bool()
        negative_mask = ~positive_mask

        large_negative_value = -1e6
        epsilon = 1e-10  # A small value to avoid log(0)

        positive_similarity = torch.where(positive_mask, similarities, torch.full_like(similarities, large_negative_value))
        negative_similarity = torch.where(negative_mask, similarities, torch.full_like(similarities, large_negative_value))
        
        exp_positive_similarity = torch.exp(positive_similarity)
        exp_negative_similarity = torch.exp(negative_similarity)
        
        # Since masked_select flattens the tensors, compute the number of positives/negatives per batch for reshaping
        positives_per_batch = exp_positive_similarity.sum(dim=1)
        negatives_per_batch = exp_negative_similarity.sum(dim=1)

        loss_tensor = positives_per_batch / (positives_per_batch + negatives_per_batch)

        log_loss = -torch.log(loss_tensor + epsilon)
        
        return log_loss

    def contrastive_loss_with_similarity(self, similarity_matrix, labels, margin=0.65):
        positive_mask = labels.bool()
        negative_mask = ~positive_mask

        positive_similarity = torch.where(positive_mask, similarity_matrix, torch.full_like(similarity_matrix, 1))
        negative_similarity = torch.where(negative_mask, similarity_matrix, torch.full_like(similarity_matrix, margin))

        # Safe subtraction for margin to prevent underflow
        safe_positive_similarity = torch.clamp(1 - positive_similarity, min=0)
        safe_negative_similarity = torch.clamp(negative_similarity - margin, min=0)

        positive_loss = F.relu(safe_positive_similarity)
        negative_loss = F.relu(safe_negative_similarity)

        combined_loss = negative_loss + positive_loss
        combined_loss = combined_loss.mean(dim=1, keepdim=True).sum()

        return combined_loss

    def ranking_similarity_loss(self, similarities, binary_mask, margin=0.1):
        ...
        positive_similarities = torch.where(positive_mask, similarities, torch.full_like(similarities, 1))
        negative_similarities = torch.where(negative_mask, similarities, torch.full_like(similarities, -1))

        # Expand dimensions for broadcasting
        # Adding an extra dimension to positive similarities so that each positive can be broadcasted
        # against all negatives in the dimension expansion
        pos_sim_exp = positive_similarities.unsqueeze(2)  # Shape: [batch_size, num_pos, 1]
        neg_sim_exp = negative_similarities.unsqueeze(1)  # Shape: [batch_size, 1, num_neg]

        # Now, perform the operation. 
        # Since we are negating positive similarities, we use '-' before pos_sim_exp
        diffs = -pos_sim_exp + neg_sim_exp + margin

        # Apply ReLU to focus on positive differences, penalizing cases where the margin isn't met
        relu_diffs = F.relu(diffs)

        instance_loss = relu_diffs.sum(dim=(1, 2))

        # Count True values for each batch
        true_counts = binary_mask.sum(dim=1).float()  # Convert to float for division

        # Handle division by zero if any batch_sum is zero
        # Replace 0 in instance_loss with 1 to avoid division by zero (or use a very small number if that's more appropriate)
        true_counts = torch.where(true_counts == 0, torch.tensor(1.0).to(true_counts.device), true_counts)

        # Divide true counts by batch_sums
        loss = instance_loss / true_counts
