...
class MultiGPUSparseAdjDataBatchGenerator(object):
    ...
    def __iter__(self):
        ...
        for a in range(0, n, bs):
            ...
            print(batch_indexes, file=sys.stderr)
            print(self.labels, file=sys.stderr)
            print(self.labels[batch_indexes], file=sys.stderr)
            print(self.device1, file=sys.stderr)
...
class DRAGON_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
        ...
        self.device0, self.device1, self.device2 = device
        ...
        *self.train_decoder_data, self.train_adj_data = self.load_sparse_adj_data_with_contextnode(train_adj_path, max_node_num, train_concepts_by_sents_list, mode='train')
        if not debug:
            for x in [self.train_labels] + self.train_encoder_data + self.train_decoder_data:
                ...
                break
                # print(len(self.train_adj_data[0]), file=sys.stderr)
    ...
    def load_sparse_adj_data_with_contextnode(self, adj_pk_path, max_node_num, concepts_by_sents_list, mode='eval'):
        ...
        if use_cache:
            ...
            with open('output.txt', 'w') as f:
                ...
                print(concept_ids, file=f)
                print(node_type_ids, file=f)
                print(node_scores, file=f)
                print(adj_lengths, file=f)
                print(special_nodes_mask, file=f)
                print((edge_index, edge_type), file=f)
...
class DRUMS_DataLoader(object):
    ...
    def __init__(self, args, train_statement_path, train_adj_path,
                 dev_statement_path, dev_adj_path,
                 test_statement_path, test_adj_path,
                 batch_size, eval_batch_size, device, model_name, max_node_num=20000, max_seq_length=128,
                 is_inhouse=False, inhouse_train_qids_path=None,
...
def simple_convert_query_examples_to_features(examples, label_list, max_seq_length, tokenizer, debug=False):
    """ Loads a data file into a list of `InputBatch`s
        `cls_token_at_end` define the location of the CLS token:
            - False (Default, BERT/XLM pattern): [CLS] + A + [SEP] + B + [SEP]
            - True (XLNet/GPT pattern): A + [SEP] + B + [SEP] + [CLS]
    ...
    for ex_index, example in tqdm(enumerate(examples), total=len(examples), desc="Converting examples to features"):
        if debug and ex_index >= debug_sample_size:
            break
        choices_features = []
        for ending_idx in enumerate(example.question):
...
def load_bert_query_input_tensors(statement_jsonl_path, max_seq_length, debug, tokenizer, debug_sample_size):
    ...
    def convert_features_to_tensors(features):
        ...
        #     print(feature.input_ids, file=sys.stderr)
        #     break
        all_input_ids = torch.tensor(select_field(features, 'input_ids'), dtype=torch.long)
        all_input_mask = torch.tensor(select_field(features, 'input_mask'), dtype=torch.long)
        all_segment_ids = torch.tensor(select_field(features, 'segment_ids'), dtype=torch.long)
    ...
    num_workers = 5
    if num_workers <= 1:
        features, concepts_by_sents_list = simple_convert_query_examples_to_features(examples, list(range(len(examples[0].endings))), max_seq_length, tokenizer, debug)
    else:
        from copy import deepcopy
        from multiprocessing import Pool
        c_size = len(examples)//num_workers +1
        examples_list   = [examples[i*c_size: (i+1)*c_size]      for i in range(num_workers)]
        label_list_list = [list(range(len(examples[0].label))) for i in range(num_workers)]
        max_seq_length_list = [max_seq_length      for i in range(num_workers)]
        tokenizer_list      = [deepcopy(tokenizer) for i in range(num_workers)]
        with Pool(num_workers) as p:
            ress = list(p.starmap(simple_convert_query_examples_to_features, zip(examples_list, label_list_list, max_seq_length_list, tokenizer_list)))
        features, concepts_by_sents_list = [], []
        for res in ress:
            features += res[0]
            concepts_by_sents_list += res[1]
